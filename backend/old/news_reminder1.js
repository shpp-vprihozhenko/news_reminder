...
app.post('/trySearch', (req, res)=>{
    console.log('/trySearch req with', req.body);
    trySearch(req.body, res);		
});

// -------------------------------------------------------------------------

async function trySearch(body, res) { 
...
async function editSearch(body, res) { 
    ...
    let newLinks = await _getLast2daysNewsByKeyWords(search.keywords, search.site, 2);
